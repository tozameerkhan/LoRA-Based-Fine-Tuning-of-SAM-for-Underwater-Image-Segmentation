...
class SUIMDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #print(label.shape)
        # If label is still RGB (i.e., shape is H x W x 3), convert to index mask
        if label.ndim == 3 and label.shape[2] == 3:
            label = self.rgb_to_mask(label)
        ...
        sample['case_name'] = self.files[idx].replace('.npz', '')
        return sample


    def rgb_to_mask(self, rgb_mask):
        COLOR_MAP = {
            (0, 0, 0): 0,      # water background
            (0, 0, 255):1,    # Human Divers
            (0, 255, 0): 2,      # Aquatic Plants/Flora
            (0, 255, 255): 3,    # Wrecks/Ruins
            (255, 0, 0): 4,      # Robots And Instruments
        ...
        for rgb, idx in COLOR_MAP.items():
            mask[np.all(rgb_mask == rgb, axis=-1)] = idx
