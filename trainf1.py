...
from trainerf1 import trainer_suim
from icecream import ic

parser = argparse.ArgumentParser()
parser.add_argument('--root_path', type=str,
                    default='./data/suim_npz1', help='root dir for data')
parser.add_argument('--output', type=str, default='/output/sam/results')
parser.add_argument('--dataset', type=str,
                    default='suim', help='experiment_name')
...
parser.add_argument('--max_epochs', type=int,
                    default=450, help='maximum epoch number to train')
...
parser.add_argument('--warmup_period', type=int, default=750,
                    help='Warp up iterations, only valid whrn warmup is activated')
parser.add_argument('--AdamW', action='store_true', help='If activated, use AdamW to finetune SAM model')
parser.add_argument('--module', type=str, default='sam_lora_image_encoder')
parser.add_argument('--dice_param', type=float, default=0.4)
parser.add_argument('--focal_param', type=float, default=0.5)  # Added focal weight param
parser.add_argument('--resume', action='store_true', help='Resume training from the latest checkpoint')
parser.add_argument('--lr_decay_power', type=float, default=0.6,
                    help='Power factor for polynomial learning rate decay')
parser.add_argument('--patience', type=int, default=50, help='Early stopping patience')
parser.add_argument('--min_lr', type=float, default=1e-6, help='Minimum learning rate')

parser.add_argument('--use_clr', action='store_true', help='Use cyclic learning rate')
...
if __name__ == "__main__":
    ...
    snapshot_path = os.path.join(os.getcwd(), 'output', "{}".format(args.exp))
    ...
    sam, img_embedding_size = sam_model_registry[args.vit_name](image_size=args.img_size,
                                                                ...
                                                                pixel_std=[1, 1, 1])
    print("img_embedding_size: ", img_embedding_size)
    pkg = import_module(args.module)
    net = pkg.LoRA_Sam(sam, args.rank).cuda()

    # net = LoRA_Sam(sam, args.rank).cuda()
    if args.lora_ckpt is not None and not args.resume:
